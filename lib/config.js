...
module.exports.getOptions = () => {
  if(cliConfig.print2page){
    mergedConfig.height = mergedConfig.height * 2;
    mergedConfig.print2page = true;
  }
 return mergedConfig;
};
...
module.exports.getSlideOptions = options => {
  if(cliConfig.print2page){
    var opt =  _.defaults({}, cliConfig, options, localConfig, defaults);
    opt.height = opt.height * 2;
    opt.showNotes = "separate-page";
    opt.print2page = true;
    return opt;
  }
...
module.exports.getRevealOptions = options => {
  if(cliConfig.print2page){
    var opt =  _.defaults({}, options, localConfig, revealConfig);
    opt.height = opt.height * 2;
    opt.showNotes = "separate-page";
    opt.print2page = true;
    return opt;
  }
  return _.defaults({}, options, localConfig, revealConfig);
};
...
module.exports.getPageOptions = printSize => {
  if (printSize) {
    ...
    if(cliConfig.print2page){
      return { width: revealConfig.width, height: revealConfig.height * 2, print2page: true};
    }
